...
def main():
    ...
    if selected_artists:
        ...
        SELECT 
            count() as total_lots,
            sum(hammer_price * fx.rate_to_usd) as total_volume_usd,
            avg(hammer_price * fx.rate_to_usd) as avg_price_usd,
            max(hammer_price * fx.rate_to_usd) as max_price_usd
        ...
        if not df_kpi.empty:
            ...
            col1.metric("Total Lots Sold", f"{df_kpi['total_lots'][0]:,}")
        ...
        SELECT 
            ...
            (s.hammer_price * fx.rate_to_usd) as price_usd,
            l.title
        FROM sothebys.sales s
        JOIN sothebys.lots l ON s.lot_uuid = l.lot_uuid
        JOIN sothebys.auctions a ON l.auction_uuid = a.auction_uuid
